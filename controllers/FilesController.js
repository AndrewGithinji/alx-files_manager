...
class FilesController {
  static async postUpload(request, response) {
    ...
    if (parentId !== 0) {
      const parentFile = await dbClient.getFile(ObjectID(parentId));
    ...
    if (type === 'folder') {
      fileInserted = await dbClient.createFile({
  ...
  static async getShow(request, response) {
    ...
    const user = await dbClient.getUser(ObjectID(keyID));
    if (!user) {
      return response.status(401).json({ error: 'Unauthorized' });
    }

    const idFile = request.params.id || '';
    const fileDocument = await dbClient.getFile(ObjectID(idFile), user._id);
    if (!fileDocument) {
  ...
  static async getIndex(request, response) {
    ...
    const files = await dbClient.getFiles(aggregateData);
